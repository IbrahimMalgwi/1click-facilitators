  ...
  return <>
    ...
    <section id="services" className="mx-auto max-w-7xl px-5 py-20 md:px-10 md:py-32">
      <div className="grid gap-12 lg:grid-cols-[.75fr_2fr]">
        ...
        <div className="divide-y rule border-t rule">
          {[
            ["01","Click Facilitators","Funeral direction, caskets, burial logistics, vaults and mausoleum care.","/services/funeral-services"],
            ["02","Click Events","Weddings, birthdays, private functions and corporate occasions.","/services/events"],
    ...
    <section className="bg-stone/50 px-5 py-20 md:px-10 md:py-28">
      <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1fr_1.35fr]">
        <div className="min-h-[420px] bg-ink p-8 text-ivory md:p-12"><p className="eyebrow text-bronze">Founder & Director</p><div className="mt-24 border-t border-white/20 pt-6"><p className="display text-5xl">Akintunde<br/>Oniti</p><p className="mt-5 text-sm text-white/55">[Upload approved founder portrait]</p></div></div>
        <div className="flex flex-col justify-center"><p className="display text-4xl leading-tight md:text-6xl">“Experience matters most when families need clarity, honesty and calm.”</p><p className="mt-8 max-w-2xl leading-7 text-ink/65">Akintunde Oniti brings more than 15 years of funeral direction and hands-on vault expertise, including former leadership and engineering roles in Ikoyi, Lagos.</p>
          <div className="mt-10 grid grid-cols-2 gap-6 border-y rule py-6 text-sm md:grid-cols-4"><p><b className="display block text-3xl">15+</b>years’ experience</p><p>Former MD,<br/>Vaults & Gardens</p><p>Engineer-trained<br/>vault builder</p><p>IBD (Institute of Business Development) Nigeria<br/>Associate</p></div>
          <Link href="/about" className="mt-8 w-fit border-b border-ink pb-1 text-sm">Read Akintunde’s profile →</Link>
        </div>
      </div>
    </section>
